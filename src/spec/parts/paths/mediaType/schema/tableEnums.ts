...
// eslint-disable-next-line max-len
const rowTemplate =
  '|{{name}}|{{required}}|{{type}}|{{format}}|{{default}}|{{minimum}}|{{exclusiveMinimum}}|{{maximum}}|{{exclusiveMaximum}}|{{minLength}}|{{maxLength}}|{{uniqueItems}}|{{pattern}}|{{nullable}}|{{readOnly}}|{{writeOnly}}|{{deprecated}}|'
...
const getPropertyType = (
  ...
  itemsType?: OpenAPIV3.ArraySchemaObjectType | OpenAPIV3.NonArraySchemaObjectType
): string => {
  if (!propertyType) {
    return ''
  }
  if (propertyType === 'object') {
    return 'object (See related table)'
  }
  if (propertyType === 'array' && !itemsType) {
    return 'array'
  }
  if (propertyType === 'array' && itemsType === 'object') {
    return 'array of objects (See related table)'
  }
  if (propertyType === 'array' && itemsType) {
    return `array of ${itemsType}s`
  }
  return propertyType
}
const getPropertyItemsType = (
  ...
  if (!propertyArraySchemaObject.items) {
    return
  }
...
const generateEnums = (properties: { [name: string]: OpenAPIV3.SchemaObject }): string => {
  if (!thereAreEnums(properties)) {
    return ''
  }

  const propertiesNames = Object.keys(properties)
  const enums = propertiesNames
    .map((propertyName) => {
      // eslint-disable-next-line security/detect-object-injection
