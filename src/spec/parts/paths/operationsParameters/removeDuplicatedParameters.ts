...
const removeDuplicatedParameters = (
  ...parametersSets: (OpenAPIV3.ParameterObject[] | undefined)[]
...
const noOccurrences = (parameter: OpenAPIV3.ParameterObject, parameters: OpenAPIV3.ParameterObject[]): boolean => {
  const parametersFound = parameters.filter(
    (searchParameter) => searchParameter.name === parameter.name && searchParameter.in === parameter.in
  )
