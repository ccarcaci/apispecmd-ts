...
const extractScopes = (securityRequirementScopes: string[], securitySchemeScopes: ScopesType): ScopesType => {
  const filteredScopes: ScopesType = Object.keys(securitySchemeScopes)
    ...
    .reduce(
      (previousValue, includedSecurityScope) => ({
        ...previousValue,
        // eslint-disable-next-line security/detect-object-injection
...
const resolveScopes = (
  ...
  if (securityRequirementScopes.length <= 0) {
...
const explodeSecuritySchemes = (
  ...
  securitySchemes?: KeySecuritySchemeType
): KeySecuritySchemeType[] => {
  if (!explodingSecurity) {
    return []
  }
  if (!securitySchemes) {
    return []
  ...
  return explodingSecurity.map((securityRequirement) =>
    Object.keys(securityRequirement).reduce(
      (previousValue, securityRequirementKey) => ({
        ...previousValue,
        // eslint-disable-next-line security/detect-object-injection
...
const securityMapper = (
  ...
  // No-ref!
  if (spec.components?.securitySchemes?.$ref) {
    return []
  }
  ...
  if (securitySchemes.length <= 0) {
    securitySchemes = explodeSecuritySchemes(spec.security, spec.components?.securitySchemes as KeySecuritySchemeType)
  }
